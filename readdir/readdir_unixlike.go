...
struct readdir_dirent* readdir_readdir(const char* filename, int* entries) {
	DIR *dir;
	struct dirent *dp;
	...
	int i = 0;
	struct readdir_dirent *vals = malloc(sizeof(struct readdir_dirent)*count);
...
func List(path string) []Dirent {
	...
	var count C.int
	ents := C.readdir_readdir(dir, &count)
	...
	/*
		From: https://stackoverflow.com/questions/28925179/cgo-how-to-pass-struct-array-from-c-to-go
	...
	*/
	...
	slice := (*[1 << 30]C.struct_readdir_dirent)(unsafe.Pointer(ents))[:count:count]
